...
def get_logger(setting):
    logger = logging.getLogger(f'setting')
    ...
    file_handler = logging.FileHandler(f'logs/{setting}.log')
...
def get_setting(args, ii=999):
    return '{}_{}_{}_{}_ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_expand{}_dc{}_fc{}_eb{}_dt{}_{}_ts{}_fr{}_{}_{}'.format(
        args.task_name,
        args.model_id,
        args.model,
        args.data,
...
if __name__ == '__main__':
    ...
    logger = get_logger(get_setting(args))
    ...
    if args.is_training == 1:
        ...
        for ii in range(args.itr):
            ...
            logger.info('>>>>>>>start training : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
            exp.train_split_valid(setting, logger)
        ...
        for ii in range(args.itr):
            ...
            exp = Exp(args)  # set experiments
            logger.info('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
            exp.test(setting, logger, test=1)
            torch.cuda.empty_cache()
