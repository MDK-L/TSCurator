...
class Exp_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if not self.args.nonautoregressive and dis != 0:
                    pred_y = pred_y[:, :-dis, :]
    ...
    def test_save(self, setting, test=0, loss_output_dir='./per_instance_losses/'):
        """
        测试模型并可选地保存每个数据实例的损失。

        Args:
            setting (str): 模型设置名称，用于加载模型和保存结果。
            test (int): 是否加载测试模式下的模型检查点。
            save_loss (bool): 是否保存每个数据实例的损失。
            loss_output_dir (str): 保存损失CSV的目录。
        """
        test_data, test_loader = self._get_data(flag='test')
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if self.args.covariate:
                    if self.args.last_token:
                        outputs = pred_y[:, -self.args.output_token_len:, -1]
                        batch_y = batch_y[:, -self.args.output_token_len:, -1]
                    else:
                        outputs = pred_y[:, :, -1]
                        batch_y = batch_y[:, :, -1]
                ...
                # 计算每个实例的损失
                loss = criterion(outputs, batch_y)
                loss = loss.detach().cpu().numpy()  # Shape: (batch_size, seq_len, features)
                # 计算每个实例的平均损失
                loss = loss.mean(axis=(1, 2))  # Shape: (batch_size,)
                per_instance_losses.extend(loss.tolist())
                ...
                if self.args.visualize and i % 2 == 0:
                    ...
                    if not os.path.exists(dir_path):
                        os.makedirs(dir_path)
                    gt = np.array(true[0, :, -1])
                    pd_pred = np.array(pred[0, :, -1])
                    visual(gt, pd_pred, os.path.join(dir_path, f'{i}.pdf'))
        ...
        loss_df = pd.DataFrame({'loss': per_instance_losses})
        loss_file_path = os.path.join(loss_output_dir, f'{setting}_per_instance_loss.csv')
        loss_df.to_csv(loss_file_path, index=False)
        print(f'Per-instance losses have been saved to {loss_file_path}')
